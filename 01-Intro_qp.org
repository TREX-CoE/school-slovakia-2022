...
qp run scf | tee h2o.scf.out
   ...
   Now we have a set of Hartree-Fock orbitals, we can run a CISD
   calculation. But we don't want to include the single- and
   double-excitations from the $1s$ orbitals. For that, we need to run
