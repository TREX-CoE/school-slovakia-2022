...
** Geometry
  ...
  Create a file with the geometry of the water molecule:
...
** Introduce and optimize a Jastrow factor
 ...
 J &=& J_{en}J_{ee}J_{een} \\
   &=& \exp \Big[ f_{en}(R_{i\alpha})+f_{ee}(R_{ij})+f_{een}(R_{i\alpha},R_{j\alpha},R_{ij}) \Big]
 \end{eqnarray*}
...
3 5 7 8 9         11 13 14 15 16     17 18 20 21 23 (c(iparmj),iparmj=1,nparmc)
  ...
  Now, in the input file, below the line where the =jastrow.start= file is specified,
  specify the name of the info of the derivatives of the Jastrow. You also
  need to add a block with different options for the optimizer as follows.
...
%endmodule
  ...
  You can reduce the number of blocks to 100, and the code will slowly increase
  the number of blocks to ~nblk_max~.
