...
** Run Diffusion Monte Carlo
   ...
   First, generate an input where you read the wave function files (careful to
   load the new Jastrow factor) and perform a short VMC calculation to generate
   the walkers for DMC.
...
%endmodule
   #+end_example
    ...
    With this block, you have generated 100 walkers per core (~vmc_nconf_new~).
...
#+begin_warning
  Make sure that you have chosen ~dmc_nstep~ about 20 times larger.
#+end_warning
