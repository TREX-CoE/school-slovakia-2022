...
qp set determinants n_states 2
  ...
  #+begin_src bash
qp set determinants n_det_max 1000
  #+end_src
...
python3 /lustre/home/filippi/Tutorial-QMC-School/trex2champ.py \
  ...
  H_{2}CO has three different atom types, so the jastrow factow file
  will be slightly different from the file for water with one extra
  line for $a$ parameters. You can start by saving the following in a file
  called =jastrow.simple=:
...
jastrow_parameter   1
 ...
 (c(iparmj),iparmj=1,nparmc) ! e-e-n H
  ...
  Start by optimizing the Jastrow factor and perform a ``quick'' optimization.
  The following champ input file (=vmc_quick.inp=) contains the parameters for such a ``quick''
  optimization.
...
%endmodule
  ...
  Move the jastrow_optimal.1.iter10 to jastrow_optimal.rough_GS and 
  ...
  Use some more MC steps to perform a stricter optimization.
...
det_optimal.1.iterX files.
   ...
   Set up a DMC calculations where you use the optimal Jastrow and 
...
** Optimization of the excited state
  ...
  energies are $E_{\rm GS}+\delta E_{\rm GS}$ and 
  $E_{\rm ES}+\delta E_{\rm ES}$, the error on $\Delta E= E_{\rm ES}-E_{\rm GS}$
  is given by $\sqrt{\delta E_{\rm GS}^2+\delta E_{\rm ES}^2$.
